...
Информационно-справочная система для работы биржи ценных бумаг. 
...
Если в базе прусутствуют встречные заявки, то при добавлении новой записи выполняются эти заявки.
...
то добавляется запись на покупку только 1 акции (10-4-5=1), а в записях на продажу исправляется количество соответственно на 0 (то есть заявки на продажу были выполнены).
...
то добавляем запись на продажу 0 акций (то есть сразу выполнить заявку), а в записях на покупку изменяется количество, соответственно, на 0 акций (по цене 93 у.е.) и на 4 акции (по цене 92 у.е.).

Поиск регистронезависимый (например, одинаковыми являются тикеры `SBERP`, `sberp`, `SberP`).
...
Если пользователь ввёл неизвестную команду, то программа сообщает, выведя строку `Unknown command: C O M M A N D`, где `C O M M A N D` — это та команда, которую ввёл пользователь. Командой считается первое слово в строке (до пробела или конца строки).
...
В остальных случаях, если пользователь ввёл неправильную команду, выводится строка `Unknown command`. 

Если при чтении базы данных из файла пользователь указал несуществующий файл, выводится строка `Can not open database`.

Если при записи базы данных в файл пользователь неправильно указал имя файла файл, выводится строка `Can not write to file`.
